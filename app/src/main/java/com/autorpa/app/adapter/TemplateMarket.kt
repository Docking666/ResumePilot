package com.autorpa.app.adapter
...
class TemplateMarket(private val context: Context) {
    ...
    companion object {
        ...
        private const val EXPORT_FILE_PREFIX = "AutoRPA_Template_"
        private const val EXPORT_FILE_SUFFIX = ".json"

        // 签名密钥，用于验证模板来源（简化版）
        private const val APP_SIGNATURE = "AutoRPA_v1"
    }
    ...
    fun importTemplate(jsonString: String): ImportResult {
        return try {
            ...
            if (exportData.formatVersion > EXPORT_FORMAT_VERSION) {
                return ImportResult(
                    success = false,
                    errorMessage = "模板版本过高，请更新 AutoRPA"
                )
            }

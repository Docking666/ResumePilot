package com.autorpa.app.llm.mcp.tools

import com.autorpa.app.llm.mcp.*
...
class ExecuteActionTool(
    ...
    override suspend fun execute(params: Map<String, Any>): MCPToolResult {
        ...
        val action = when (actionType) {
            "click" -> com.autorpa.app.engine.Action.Click(
                x = (params["x"] as? Number)?.toInt() ?: 0,
                y = (params["y"] as? Number)?.toInt() ?: 0,
                description = description
            )
            "long_click" -> com.autorpa.app.engine.Action.LongClick(
                x = (params["x"] as? Number)?.toInt() ?: 0,
                y = (params["y"] as? Number)?.toInt() ?: 0,
                description = description
            )
            "swipe" -> com.autorpa.app.engine.Action.Swipe(
            ...
            "type" -> com.autorpa.app.engine.Action.Type(
                text = params["text"] as? String ?: "",
                description = description
            )
            "back" -> com.autorpa.app.engine.Action.Back
            "home" -> com.autorpa.app.engine.Action.Home
            "scroll" -> com.autorpa.app.engine.Action.Scroll(
                direction = if ((params["direction"] as? String) == "up")
                    com.autorpa.app.engine.Action.ScrollDirection.UP
                else com.autorpa.app.engine.Action.ScrollDirection.DOWN,
                description = description
            )
            "wait" -> com.autorpa.app.engine.Action.Wait(
...
class ScriptManagerTool(
    private val db: com.autorpa.app.data.db.AppDatabase
) : MCPTool {
    ...
    override suspend fun execute(params: Map<String, Any>): MCPToolResult {
        return when (params["operation"] as? String) {
            ...
            "create" -> {
                ...
                db.scriptDao().insertScript(com.autorpa.app.data.db.ScriptEntity(
 ...
 */
class ResumeTool(
    private val resumeManager: com.autorpa.app.resume.ResumeManager
) : MCPTool {

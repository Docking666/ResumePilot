...
import com.resumepilot.app.adapter.WorkflowResult
import com.resumepilot.app.data.db.ExecutionLogEntity
import com.resumepilot.app.data.db.StatsEntity
...
import com.resumepilot.app.resume.ResumeData
...
class BatchScheduler(
    ...
    suspend fun runPipeline(
        ...
        for (i in 0 until config.maxApplications) {
            ...
            val greeting = if (config.customGreeting.isNotBlank()) {
                com.resumepilot.app.resume.GreetingMessage(
    ...
    private suspend fun updateDailyStats(
        ...
        try {
            ...
            val existing = app.database.statsDao().getStatsByDate(today).firstOrNull()
